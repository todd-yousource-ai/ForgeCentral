...
function recordingClient(overrides: Partial<CrucibleClient> = {}): {
  ...
  const client: CrucibleClient = {
    ...
    objectList: (req) => {
      calls.push(`objectList:${String(req.request_id)}`);
      reads.push(req);
      return Promise.resolve({ objects: [] });
    },
    idamConnectors: (req) => {
      calls.push(`idamConnectors:${String(req.request_id)}`);
      reads.push(req);
      return Promise.resolve({ connectors: [] });
    },
    idamSync: (req) => {
      calls.push(`idamSync:${String(req.request_id)}`);
      reads.push(req);
    ...
    idamConfigure: (req) => {
      calls.push(`idamConfigure:${String(req.request_id)}`);
      reads.push(req);
      return Promise.resolve({ commit_version: 0 });
    },
    objectDetail: (req) => {
