...
**Status:** DRAFT (authored 2026-07-07; amended 2026-07-21). Inherits `TRD-CONSOLE-00`. The Users
...
| **Origin** | Local / a federation connector | the identity's authoritative source (TRD-35 Section 9); replaces the mock's deleted Override column |
...
committed through the engine's atomic batch. Local principals exist for RBAC alignment -- they are the
...
- Click a principal row -> the drawer (identity, policies, recent decisions, actions).
...
- Add/edit/status/group operations commit through the engine with audit and are tier-gated +
  confirm-gated; an edit to an IdAM-owned identity field is refused with the typed error; a connector
  sync performs a real federation and reports its real result.
...
Missing failure path: unauthorized-create, IdAM-owned-field edit refusal, revoke, failed-sync, `PENDING` tested. Dead code: every
