...
Status: **COMPLETE (Phase 1 Overview) 2026-07-17 -- the flagship surface is live, interactive, and E2E-capstoned. O1.1-O1.N all LANDED (grounded-reshape Sankey RD.1-RD.5, <=3-click interaction PR-1..PR-2c-2, O1.7 live deltas PR-3, overflow honesty PR-4, entity connections + §8 capstone PR-5). The full `TRD-CONSOLE-01` §8 acceptance is proven by the live Playwright suite. DEFERRED (named): VTZ-ring -> zone -> policy jump (needs `TRD-CONSOLE-02` zone nav); the crdb push-stream `overview.live` (Part B) swaps in behind the poll later; admin-editable VTZ assignment; loopback-dest rollup (operator decision pending).**
...
| O1.N | INV-CONSOLE-OVERVIEW-COMPLETE | **LANDED** | PR-5a `967ebce` + `overview.spec.ts`/`shell.spec.ts` | The §8 acceptance is met by the live Playwright suite: no fabricated element (the fixtureless empty-tenant render shows "No connectivity observed", `shell.spec` + surface test); a connection appears/updates < 2 s (the O1.7 live poll, `overview.spec`); hover highlights the real path + click opens the drawer with real data (design/surface hover tests + the inspect journey); the Section 5 canonical tasks -- inspect an entity + its live connections (PR-5a) and isolate an agent -> confirm -> audited receipt -- complete in <= 3 clicks. DEFERRED (named): the VTZ-ring -> zone -> policy jump (needs `TRD-CONSOLE-02` zone nav); the "filter to AI Agents" lane tab was superseded by the container-open interaction (a click into the lane's members). |
...
| **PR-5** ✅ **LANDED** (5a `967ebce` + capstone in `overview.spec`/`shell.spec`) | O1.N | `INV-CONSOLE-OVERVIEW-COMPLETE` | Shipped as **PR-5a** (the substantive gap) + the capstone e2e (already present across `overview.spec.ts`/`shell.spec.ts`). PR-5a wired the entity's live connections into the drawer: `useEntityConnections` reads `/api/overview/entity-connections` by the member's LEG kind+id; `EntityDrawer` gains an optional Connections section (absent for a non-connectivity entity, honest empty for a sink, per-section error, list otherwise) -- completing the §5 "inspect + live connections" task AND consuming PR-1's orphaned route. Capstone: Playwright proves both flagship journeys + the fixtureless empty-tenant render + the <2s live delta; §8 rows checked off (see O1.N). VTZ->policy jump DEFERRED to `TRD-CONSOLE-02`. | Playwright green on both journeys + empty-tenant + the <2s live delta; design Connections section (absent/list/empty/error); §8 rows checked off. |
