...
| CD-13 | S2 | FC | `GET /api/logs` ignores `offset`; the background pager receives page 0 repeatedly | `forgecentral/apps/bff/src/server.ts:429-464`; `forgecentral/apps/console/src/surfaces/useLogs.ts:28, 93` | VERIFIED; FIXED by GD.9 (the gateway forwards `offset`; tested) |
...
| CD-18 | S2 | crdb | Registry text the Console shows verbatim is wrong for editable rows: `sessions.*` and `cognition.max_connections` say Live but bind at start; `governance.dual_control` is live for the wire only; `idam.connector` has no consumer; `detection_retention` says Pending but is live; `lug.exposure`'s default says disabled | `crucible/crates/cdb-server/src/admin.rs:50-66, 2250-2258`; `crdb-settings-plane.md` 1.4 | VERIFIED (sessions); FIXED by `IP-CONSOLE-11-guide` GD.E0 (crdb `c31d9be4`; every row re-verified against the code first) |
| CD-19 | S2 | crdb | The Overview's zone grouping is a hard-coded demo configuration (three `Demo.*` zones; entity-id substring rules for "claude" / "codex"), not operator configuration | `crucible/crates/cdb-cyber/src/connectivity.rs:657-690` | VERIFIED; GD.10: the assignment matches entity-id substrings (claude, codex, demo-agent) with a catch-all Demo.Users.Public, and authored zones render as flowless rings that always read Nominal |
...
| CD-33 | S3 | FC | Stale UI copy: the empty plan says "nothing proposes one yet"; the Security tab says key issuing is edited on Configuration (it is pending; no form) | `fc-soc-logs-reports-settings.md` L-03 item 2, D-11 item 4 | code-read; FIXED by GD.8 (the empty-plan line now says the engine proposes at the response tier) |
...
| CD-46 | S3 | FC | Isolate's success message says the disposition is "distributed to the endpoint"; no endpoint path exists for it (the engine only records it) | `forgecentral/apps/console/src/shell/DrawerHost.tsx:195-200`; `torch-endpoint.md` 0.1 item 6 | VERIFIED (copy); FIXED by GD.10 (the line says nothing is sent to an endpoint and follows the reply's enforcement flag) |
| CD-47 | S3 | crdb + FC | Settings > Configuration's version line and each row's Source ("committed at version N") show the engine store's LATEST version, which moves with every data write, not the version the configuration was committed at; they disagree with the commit receipt and with Settings > Changes as soon as any data is ingested | `crucible/crates/cdb-server/src/handler.rs:4261-4263` (SETTINGS_READ reports `latest_version()`); `config_store.rs` (commit returns the store's commit version) | VERIFIED (GD.3, 2026-09-25) |
| CD-48 | S3 | crdb | Application, Certificate, Script and Data Store objects never resolve members: `leg_kind_for` maps them to no observed source, so their member list is always empty and the guide must say so | `crucible/crates/cdb-cyber/src/object_resolve.rs:41-61, 150-152` | VERIFIED (GD.5, 2026-09-25) |
| CD-49 | S3 | FC | The policy form's Subjects and Targets pickers key objects by selector value, so two objects with the same value collapse into one | `forgecentral/apps/console/src/surfaces/PolicyForm.tsx:151, 260-278` | VERIFIED (GD.5, 2026-09-25) |
| CD-50 | S4 | FC | A PENDING drawer section shows the binding's gating task verbatim, exposing internal plan ids to operators (e.g. the object drawer's governing-policies section) | `forgecentral/apps/bff/src/engine/entity-detail.ts:231-235`; `packages/design/src/components/EntityDrawer.tsx:123` | VERIFIED (GD.5, 2026-09-25) |
| CD-51 | S3 | FC | The Objects form reported an expired session (401) and an unreachable engine (502 / 503) as "The engine refused the command." | `forgecentral/apps/console/src/surfaces/ObjectsSurface.tsx` (commandFailure) | FIXED by GD.5 |
| CD-52 | S2 | FC | Every policy Delete from the Console reported a failure: the engine's delete ack carries no version or lifecycle, the BFF read it as a lifecycle change and returned 503, and the list did not refresh (the BFF test mocked a lifecycle the engine never sends) | `forgecentral/apps/bff/src/engine/policies.ts` (resolveDeletePolicy); `crucible/crates/cdb-server/src/handler.rs:6405-6410` | FIXED by GD.6 |
| CD-53 | S3 | FC | The policy form lists every object in both Subjects and Targets, but the engine refuses User, Group and Agent as targets and URI, Network, Registry Key and Data Store as subjects; a wrong pick fails only at save | `forgecentral/apps/console/src/surfaces/PolicyForm.tsx:259-279`; `crucible/crates/cdb-types/src/forge_v2.rs:340-386` | VERIFIED (GD.6, 2026-09-25) |
| CD-54 | S3 | crdb | The convergence ledger's Applied means the endpoint applied that version at some point, not that it is in force: an endpoint that switched to another zone's newer bundle still reads Applied for the old zone | `crucible/crates/cdb-cyber/src/apply_report_store.rs:82, 205-208` | VERIFIED (GD.6, 2026-09-25) |
| CD-55 | S3 | FC | The Users and group forms reported an expired session (401) and an unreachable engine (502 / 503) as "The engine refused the command.", and an Edit conflict (record gone, or an IdAM-owned field) as a duplicate username | `forgecentral/apps/console/src/surfaces/UsersSurface.tsx` (commandFailure, group create) | FIXED by GD.7 |
| CD-56 | S2 | FC + crdb | The connector's `running` means the sync loop is alive (set at loop start, cleared at exit), not a sync in flight; the Console read it as Syncing, so a live connector read Syncing permanently, masked its errors and completeness, and kept Sync Now disabled (the e2e fixture used a `running: false` the engine never sends) | `crucible/crates/cdb-ingest/src/idam_auth0_engine.rs:994,1014`; `packages/contracts/src/users.ts` (deriveIdamState); `IdamConnectorsPanel.tsx` | FIXED in the Console by GD.7 (the engine field keeps its meaning) |
| CD-57 | S2 | crdb | A connector onboarded from ForgeCentral on a node installed without an Auth0 domain syncs into the nil tenant: the boot config's connector tenant is set only when `CDB_AUTH0_DOMAIN` is given at install, and only boot validation rejects a nil tenant for an enabled connector; synced users never appear on All Users | `crucible/crates/cdb-server/src/bin/cdb-mkconfig.rs:917-926`; `config.rs:1760-1762, 1812-1816`; `bootstrap.rs:5963-5982` | VERIFIED by code read (GD.7, 2026-09-25); live readout owed |
| CD-58 | S4 | FC | The External IDAM empty state and its Onboard Auth0 action are unreachable: the engine always returns one connector record (disabled, empty domain) on an unconfigured node | `crucible/crates/cdb-server/src/bootstrap.rs:1141-1162`; `IdamConnectorsPanel.tsx:243-256` | VERIFIED (GD.7, 2026-09-25) |
| CD-59 | S3 | FC | The SOC surface's `Enforcement off` pill, the Auto-Contained badge and the "Already enforced" copy are constants, not reads; they will be wrong on an enforcement-on node | `SocOpsSurface.tsx:99, 174`; `SocVerdictPanel.tsx:383` | VERIFIED (GD.8, 2026-09-25) |
| CD-60 | S3 | FC | SOC copy claimed what does not happen: the disposition dialogs said every verdict is a calibration label and that a false positive down-weights the rule (it is the technique) and cannot be undone (a later verdict replaces it); "panels re-read" while nothing polls; the command lines read 401 and 502 as "did not reach the engine" and every 503 as unrenderable | `SocCaseControls.tsx`, `SocVerdictPanel.tsx`, `use{Case,Plan}Command.ts`, `useCognitionRun.ts`; `crucible/crates/cdb-cyber/src/detect_feedback.rs:112-128, 273-281` | FIXED by GD.8 |
| CD-61 | S3 | crdb | In episode mode LOG_EXPORT reads legacy decisions plus episodes while LOG_QUERY reads episodes only, so an export can contain rows the table never showed | `crucible/crates/cdb-server/src/handler.rs:7258-7268, 7322-7340` | VERIFIED by code read (GD.9, 2026-09-25) |
| CD-62 | S3 | FC | The Logs time range has no end bound: every preset compiles to `since` only, so a narrower range never reaches older rows, and `since` goes stale while the page is open | `forgecentral/apps/console/src/surfaces/LogsSurface.tsx:76-86` | VERIFIED (GD.9, 2026-09-25) |
| CD-63 | S2 | FC + crdb | The drawer's Recent decisions is always empty: the gateway queries ENTITY_DECISIONS with `entity_type: principal`, a kind the engine does not index (host, user, ip, process, hash, mac, interface), so it returns nothing | `forgecentral/apps/bff/src/engine/entity-detail.ts:298-302`; `crucible/crates/cdb-server/src/handler.rs:7092-7138` | VERIFIED by code read (GD.10, 2026-09-25); the fix needs the engine's subject-to-kind mapping |
| CD-64 | S3 | FC | The Isolate result line rendered outside the drawer panel, under the drawer's full-screen overlay, so the operator likely never saw it; its failure line read every status as a refusal | `forgecentral/apps/console/src/shell/DrawerHost.tsx`; `packages/design/src/styles.ts:102-108` | FIXED by GD.10 (rendered inside the drawer; per-status lines; tested) |
| CD-65 | S2 | FC | One slow engine read fails every read queued behind it: the BFF serializes all engine calls on one connection, a call that exceeds `requestTimeoutMs` (5 s) invalidates the connection, and the queued calls then spend both attempts on the closed transport. Live 2026-09-25 20:31:14.937: the Overview graph read (started 20:31:04.9) timed out twice and took down four Settings reads at the same millisecond (browser saw a 502; header showed Not live); 1 of 2 Overview reads failed in that hour, 5 in the prior day | `forgecentral/apps/bff/src/engine/wire-client.ts` (`serialize`, `call`, `invalidate`) | VERIFIED live (GD.N, 2026-09-25); not fixed (the connection model is outside the guide plan) |
| CD-66 | S3 | FC | A disabled primary button kept its brand fill at reduced opacity, so it read as available (seen live on the policy form's Save & Publish) | `forgecentral/apps/console/src/shell.css` | FIXED by GD.N (disabled primary drops the fill; e2e asserts the computed background) |
