...
| 3 -- Governance CRUD | `CONSOLE-02` VTZ, `CONSOLE-10` Objects, `CONSOLE-04` Users, `CONSOLE-05` Policies | read/author/publish surfaces; each reuses the drawer. **Resequenced engine-first (2026-07, operator):** VTZ (COMPLETE) -> Objects (COMPLETE) -> Users + IdAM (COMPLETE) -> **Policies (the access contract, LAST -- it binds the nouns the others establish; ACTIVE)**. Policies: `IP-CONSOLE-05-policies.md` (P5.1-P5.N) over the crdb engine half `crdb IP-CONSOLE-POLICY-SUBSTRATE` (PS.1-PS.N, engine-first); re-homes the `IP-CONSOLE-02-FORGE-DISTRIBUTION` FD.7c compose->sign->push + convergence ledger onto the Policy tab (the 2026-07-21 surface-placement rule); host realization of the new rule dimensions is `torch IP-TORCH-POLICY-ENFORCE` (gated, AG.7-OFF). **COMPLETING `IP-CONSOLE-02-FORGE-DISTRIBUTION` (crdb convergence-read delegation + torchd `TORCH_POLICY_ANCHOR` provisioning + the FD.N live capstone) is a NAMED PREREQUISITE of full Forge policy PREVENTION (operator directive 2026-07-24; the P5.N live drive proved the producer half and surfaced both gaps -- see the directive block in that IP).** |
...
| 6 -- Administration | `CONSOLE-11` Settings | the admin surfaces on the Phase-0 admin plane; last because it is operator config over already-exposed engine admin surfaces (**`IP-CONSOLE-11-settings.md`**, the ten tabs). The in-app configuration guide, the ReadMe tab under Settings covering every configuration on every surface, is **`IP-CONSOLE-11-guide.md`** (2026-09-25, from the configuration census `IP-CONSOLE-11-guide-CENSUS.md`; complete). The census defects it did not fix (54 rows, across the Console, the engine and the endpoint agent) are planned in **`IP-CONSOLE-11-defect-remediation.md`** |
...
| `policies.*` reads + commands (`CONSOLE-05`) | the Forge policy store (per-VTZ rulesets, network match, restrictions, applied-to) | crdb -- `IP-CONSOLE-POLICY-SUBSTRATE` PS.1-PS.N (`POLICY_LIST_BY_ZONE`/`DETAIL`/`CREATE`/`EDIT`/`PUBLISH`/`DELETE` + the PS.7 composer seam); PS.5 unblocks P5.1 |
| runtime enforcement of schedule/geo/ports (`CONSOLE-05`) | the host realizing the authored rule dimensions | torch -- `IP-TORCH-POLICY-ENFORCE` PE.1-PE.N, **gated on the enforcement toggle (AG.7-OFF)**; authoring/distribution/audit are real without it |
