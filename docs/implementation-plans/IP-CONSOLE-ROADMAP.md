...
| 5 -- Brokered plane | `CONSOLE-06` TrustFlow | the sensor-tap flows land now; the `llm.*`/`mcp.*` inference panels are `PENDING` behind the Torch trustflow-proxy live leg (Section 6) |
...
| `trustflow.inference` -- `llm.*`/`mcp.*` (`CONSOLE-06`) | the inference-intent tap | torch -- stand up the `torch-trustflow` proxy live + route agents through it + land the tap into the govern envelope (the deferred-live leg; see the torch IP-TORCH-AUTOGOVERN adjacent work). **Hard gate: never fabricate inference content.** |
