...
| F0.8b | INV-CONSOLE-SHELL-3-CLICK-FRAME (e2e) | OPEN | -- | The tier-4 empty-state E2E (Playwright): login -> shell -> open the drawer host, driven in a browser against a mocked BFF. Split from F0.8a because Playwright carries a browser-download postinstall + supply-chain-allowlist handling (the gate's step 8 is already `--skip-net`/`--skip-e2e` gated); lands as its own PR per the repo's a/b precedent. F0.1 deferred Playwright to here. |
