...
Status: **F0.1 + SC + F0.2a/b + F0.2c (drawer + confirm shells) + F0.3 (+ F0.3b native wire transport, LIVE-PROVEN) + F0.4 (no-stub gate) COMPLETE; F0.5a/F0.5a-2 OIDC auth LIVE-PROVEN; F0.5b operator Principal + engine facade LANDED; F0.5c wire + producer LANDED (crdb CD.1/CD.1a consume-side LANDED + LIVE-PROVEN, crdb `6746ce8f`) -- its live end-to-end join is batched into the Phase 0 exit capstone. F0.6 DEFERRED (product owner). F0.7 admin plane MOVED to `IP-CONSOLE-00-CRYPTO-SIDECAR` (AWS-LC everywhere: a Rust loopback sidecar terminates admin 8443 + originates engine :7878; Node performs no TLS; revises F0.3b transport). F0.8 COMPLETE: F0.8a SPA shell (`apps/console`: IA + routing + drawer host + query/live-store + error boundary + the four states, no surface data; 17 tier-1/2 tests) + F0.8b the Playwright empty-state e2e (chromium, mocked BFF; 2 tier-4 journeys; gate step 8 active). Finishing F0: the crypto-sidecar IP capstone (CS.N), then the full Phase 0 live run.**
...
| F0.3b-3b | INV-CONSOLE-WIRE-TRANSPORT | LANDED (review) | e1e73dd | `StreamFrameTransport` (frame reassembly over a duplex) + `connectTls` mTLS dial; handshake proven over the real framed transport. **REVISED by CS.4 (`IP-CONSOLE-00-CRYPTO-SIDECAR`):** the `node:tls` dial moves behind the AWS-LC sidecar (plaintext loopback); the frame/CBOR/handshake stack is unchanged and still byte-exact to crdb. |
...
| F0.5c-producer | INV-CONSOLE-ENGINE-AUTHZ | LANDED (review) | 9a7fa4f + 24bff4f | RBAC v1 (`resolveAuthority`) + `operatorPrincipalId` (v5 UUID); login resolves the operator's tenant + role + principalId into the session (fail-closed 403 when no authority); `OperatorEngine.querySubmit` injects `operator:{principal,tenant}` onto the `WireQuerySubmit`. `FC_RBAC_CONFIG` (JSON, zod-validated) is the v1 store. **Remaining: live end-to-end** (add `WirePlane::Delegation` to the live node's console-bff peer + seed an operator tenant + a groupRoles map, then operator login -> tenant-scoped read). |
...
| F0.6 | INV-CONSOLE-LIVE | DEFERRED | -- | Live-feel channel (v1: short-interval CrucibleQL polling). **Deferred (product owner 2026-07-10):** not a foundation prerequisite -- surfaces manage via admin ops + observe via CrucibleQL reads (the same reasoning that banked the crdb push-stream). Revisit when a demanding surface (realistically the Overview graph at scale) needs sub-2s live updates. |
| F0.7 | INV-CONSOLE-ADMIN-PLANE | MOVED | -- | 8443 node-IP admin listener; hybrid-PQC + CNSA-1.0 floor. **Realized by `IP-CONSOLE-00-CRYPTO-SIDECAR` (CS.2 + CS.4):** terminated in the AWS-LC sidecar, not Node/OpenSSL (product-owner decision 2026-07-10 -- AWS-LC everywhere; Node performs no TLS). The Node/OpenSSL cut was discarded before merge. |
| F0.8a | INV-CONSOLE-SHELL-3-CLICK-FRAME | LANDED (review) | (this PR) | The SPA shell (`apps/console`, `@forge/console-spa`): React 19 + Vite, the left-rail IA (all eleven destinations from a single `ia/destinations` source, one route each), the drawer host (composes `@forge/design` `Drawer`; the select-then-act frame reachable now, entity body is TRD-CONSOLE-12), the query layer (TanStack Query) + the auth gate (`/auth/me` -> login (device flow) or shell), the live-store (stream-shaped; F0.6 deferred so it reports an honest `unavailable`/stale, never a fake "Live"), an error boundary, and the four explicit states (loading skeleton / honest empty / engine-typed error+request-id / staleness marker). Renders the design system; **no surface data** (every destination is an honest empty state, proven by `test:contract`). Tests: tier 1/2 (17 cases: rail renders all 11, one-click navigate + active state, drawer open/close, account menu + tier, not-found, states + boundary, auth gate login-vs-shell). New deps (scope-sanctioned by the roster: routing + TanStack Query + the design system): `react-router-dom`, `@tanstack/react-query`, `vite`/`@vitejs/plugin-react`, testing-library. License gate: a reviewed dev-only/data-only exception for `caniuse-lite` (CC-BY-4.0, build-time browser-compat data, never in `dist/`) added to `check-licenses.mjs` + `DEPENDENCY-POLICY.md`. Full `scripts/ci.sh` green (steps 1-10; sidecar Rust unaffected). |
| F0.8b | INV-CONSOLE-SHELL-3-CLICK-FRAME (e2e) | LANDED (review) | (this PR) | The tier-4 empty-state E2E (Playwright, chromium) driving the built shell in a real browser with the BFF mocked at the network boundary (`page.route`), so it proves the shell and fabricates no engine data. `apps/console/playwright.config.ts` (chromium project; `webServer` = the Vite dev server on 127.0.0.1:4173) + `e2e/shell.spec.ts` (two journeys: unauthenticated -> the login screen, no shell; authenticated -> the rail's 11 destinations, the honest Overview empty state + "Not live", open/close the drawer frame, one-click navigate to Policies -> its empty state). Wiring: `@playwright/test` devDep (Apache-2.0; **Playwright 1.49 carries NO install script**, so no `onlyBuiltDependencies` entry -- the browser is fetched by an explicit `playwright install chromium` in `test:e2e`, not on npm install); vitest `exclude` gains `e2e/**` so the two runners do not overlap; the app tsconfig includes `e2e`; the CI workflow provisions the browser + OS libraries with `playwright install --with-deps chromium` (needs the runner's sudo, kept out of the shared `ci.sh`). Full `scripts/ci.sh` green with step 8 ACTIVE (2 e2e passed in-browser). Supersedes the F0.8b OPEN note; INV-CONSOLE-SHELL-3-CLICK-FRAME is now proven at tiers 1/2 (F0.8a) + 4 (here). |
