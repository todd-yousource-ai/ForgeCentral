...
| FD.7c | `INV-CONSOLE-FORGE-CONVERGENCE-VISIBLE` (FC) | The ledger surface: a BFF projection over `BUNDLE_CONVERGENCE` (never a Console-held table, `INV-CONSOLE-NO-2ND-DB`) and the SPA view of the three states, reachable <=3 clicks from a zone. The typed `ApplyError` is carried through, never collapsed to "failed". |
