...
- **ID.4a LANDED** (`c2bba2d`): the two cadence controls are wired end to end (`IDAM_CONFIGURE` BFF route
  + the onboarding form's bounded numeric inputs + the card's live poll interval; the engine holds the
  bound). The IdAM CONSOLE surface (ID.1-ID.5, ID.4, ID.4a) is now COMPLETE.
- **Remaining IdAM work:** (1) The **live capstones** -- crdb DR.N (attributed directory) + CO.N (the connect re-spawn asserting the
...
| ID.4a | `INV-CONSOLE-IDAM-CADENCE-EDITABLE` | **LANDED** | `c2bba2d` | IDAM_CONFIGURE wired (BFF `POST /api/idam/configure` + resolver); the onboarding form has the two bounded cadence controls (poll 60-86400s, full 1-168h) and the card shows the live poll interval; the engine holds the bound (out-of-range refused server-side, proven by a test) |
