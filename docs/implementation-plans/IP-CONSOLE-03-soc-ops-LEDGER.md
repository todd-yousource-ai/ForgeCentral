...
| S3.10 | A3 | ON BRANCH `feat/console-03-coverage-kpi` (2026-09-23) | -- | "N of M rules evaluable on your sources" (crdb IP-AISOC-STEP1 B.8 / B.8a, `INV-DET-COVERAGE-VISIBLE`): the wire schema re-vendored from crdb `58cc5197` (`WireDetectSummary.coverage?`, `WireDetectCoverage`, `WireCoverageBlocker`; `pnpm generate`), `SocKpis.coverage: SocCoverage \| null` projected from the same DETECT_SUMMARY payload the strip already reads (no new binding), a sixth KPI tile **Rules Evaluable** = `evaluable of rulesLoaded` with the ONE most-unlocking blocker as the sub-line (`<logsource> would unlock N`, `(list cut)` when the node stated a cut) and the corpus source as the badge; an absent `coverage` (a pre-B.8a node) renders **Not reported / Older node**, never a zero. Tests: contracts (projection with ranked blockers; absent -> null) + surface (the tile reads `24 of 49` and the blocker; the not-reported state). |
