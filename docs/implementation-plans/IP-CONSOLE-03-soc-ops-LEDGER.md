...
- **State (2026-09-24, 01:35 UTC): S3.11 LANDED `52c9f31` + DEPLOYED 01:33 UTC** -- the BFF half of the analyst loop (case acts + notes + disposition over crdb C.1 / SC.7; the `dispositioned` trail-narrower defect closed). Installer inputs that worked: `CONSOLE_NODE_IP=172.31.1.36 CONSOLE_BIN_DIR=/var/lib/cdb-models/cdb-build/release CONSOLE_PEER_TENANT=df46dcb7-2e91-448c-a406-42e492b85e36` (it re-pins the console peer and restarts cdb; restart torchd after). Owed: an operator-session look at the Audit Trail pane for the C.1 capstone incident (six acts). **NEXT = S3.12, the SPA controls** (Assign / Ack / Note / Close / Disposition on the verdict panel, confirm-gated like Approve; a Notes pane in the dock; the trail rendering the new acts; do not infer sequence from same-second trail order). Open design question for S3.12: the assignee picker -- RBAC is group-based in the BFF, so members are not enumerable there; candidates are the engine Users directory (LUG humans) or an IdP group-members read.
