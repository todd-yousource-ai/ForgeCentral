...
- **State (2026-09-24): S3.11 ON BRANCH `feat/console-03-case-acts` `eb07b92`, gate green, AWAITING REVIEW** -- the BFF half of the analyst loop (case acts + notes + disposition over crdb C.1 / SC.7; see the S3.11 row incl. the `dispositioned` trail-narrower defect it closes). After merge: deploy the BFF (install.sh / the Console deploy), verify `GET /api/soc/audit` for the C.1 capstone incident now renders its six acts, then S3.12 (the SPA controls). S3.10 flipped LANDED (`e7d5ba2`).
