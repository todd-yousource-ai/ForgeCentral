...
| `idam.configure` | `PENDING`; `Configure` is a labelled disabled control | **LIVE** on `IDAM_CONFIGURE`; non-secret fields + a secret **reference** only, including both editable cadences (ID.4a) |
...
| **ID.4** | `INV-CONSOLE-IDAM-CONFIGURE-SAFE` | **RESCOPED by operator ruling 2026-07-23 (see LEDGER Operator directives + IA.9): the form now collects the FULL connectivity (domain, client id, audience) AND the client secret; the secret TRANSITS to the engine and is stored server-side (encrypted at rest, never logged), write-only from the console. This reverses the reference-only design described in the rest of this row, which is retained for history but superseded. ID.4 is engine-first on crdb IA.9.** `Configure` becomes real. BFF: the `IDAM_CONFIGURE` codec + delegated action + `POST /api/idam/configure`, fail-closed draft parse. SPA: the Configure form collects **domain, client id, audience, secret reference, poll interval, full-sync cadence, enabled** -- with the secret-reference field labelled as *a path to a secret already placed on the node*, and inline copy stating the Console never transmits the secret itself. Tier-gated (Admin/SecurityAudit) and confirm-gated. A submission that would carry a literal secret is impossible by type and additionally refused by the engine (proven by a test asserting the exact POST body shape). |
| **ID.4a** | `INV-CONSOLE-IDAM-CADENCE-EDITABLE` | **The two cadences are first-class, editable controls** (operator directive 2026-07-22). The Configure form renders **Delta poll interval** (default 300s, range 60-86400) and **Full directory sync cadence** (default 24h, range 1-168h) as bounded numeric inputs with their units, their ranges shown, and the current engine values as the initial state -- never a hardcoded placeholder. The connector CARD surfaces the live interval alongside state and last-sync, so an operator sees the cadence without opening the form. The engine's typed out-of-range refusal renders as a field error; the form's own range hints are UX only, and a test drives an out-of-range value straight at the BFF to prove the engine still refuses it (client-side gating is not the bound). A saved change takes effect with no restart, and the card reflects the new value on refetch. |
...
need ID.2's resolver. ID.4a follows ID.4 (same form) and needs crdb IA.7's bounded, runtime-applied
cadence knobs. ID.5 needs IA.6 engine-side. ID.N needs everything plus a real synced tenant.
