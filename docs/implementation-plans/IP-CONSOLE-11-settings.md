...
| SET.2 / SET.2b | `SETTINGS_COMMIT`: scalar knob edits (SET.2) and typed patches over the LIVE sections -- dual control, egress destinations, LUG exposure, decoder families, source-format map, narrative model ref (SET.2b); boot-bound and pending settings refused / absent | ST.2, ST.7, ST.8b |
...
| **ST.3** | 9.2 RBAC | **The RBAC tab** (after SET.1): the `admins` assignments (identity, roles from the engine's `AdminRole` set, clearance) and `sso_group_roles` READ-ONLY with the boot-bound label and the `cdb-actl` + restart path stated (crdb SET.2b found both boot-bound); the Console's own operator roles (`global-admin` / `tenant-admin` / `tenant-user`, from BFF `rbac.ts`) shown read-only and labelled as installer configuration; the tenant operators (Users surface) linked, not duplicated. |
| **ST.4** | 9.2 Federation | **The Federation tab**: (a) the IdAM connector panel mounted from the Users surface (one component, two mounts; no second fetch path), LIVE now; (b) the `sso_group_roles` map READ-ONLY (boot-bound; after SET.1). |
...
| **ST.7** | 9.2 Policy | **The Policy tab** (after SET.2b): the narrative model ref editable; the detection watermark bounds, detection retention and served models READ-ONLY with the pending label (crdb SET.2b found no live consumer); credibility weights read-only with the version and the reason (replay-pinned); the response tiers linked to the SOC tab (one form, one home). |
