...
- The live-feel channel delivers a fresh delta to the browser end to end against the seeded demo tenant
  by **short-interval CrucibleQL polling** (the engine push-stream is banked/deferred, Section 5 F0.6);
  the channel is stream-shaped so it can swap to the push-stream later without touching surfaces.

The Phase-0 invariants proven: `INV-CONSOLE-NO-STUB`, `INV-CONSOLE-NO-2ND-DB`, `INV-CONSOLE-ENGINE-AUTHZ`,
`INV-CONSOLE-ADMIN-PLANE`, `INV-CONSOLE-LIVE` (v1 via bounded-interval polling), and the `<=3-click` frame
...
| **F0.6** | **The live-feel channel (v1: polling).** The BFF refreshes the live surfaces (Overview graph, Logs/AIOps feeds) by **short-interval CrucibleQL polling** (`AS OF`/incremental over `QuerySubmit`), fanned to the browser over a single push channel (**SSE** default), backpressured, with reconnect + **resync-from-engine** (never from cache); a "Live" badge + a staleness marker on lag. The interface (the BFF channel, the SPA live-store) is stream-shaped so the engine push-stream (crdb Part B, **banked/deferred**) can replace polling later without touching surfaces. **No fabricated data**: the poll reads real committed state. | `INV-CONSOLE-LIVE` -- a live panel reflects engine state within < 2 s with an explicit staleness indicator; v1 meets it by bounded-interval polling, not a push snapshot on a timer with no freshness guarantee. | 2 (channel lifecycle: poll/backpressure/reconnect/resync), 5 (feed load) |
...
|------------------------|----------------------------|----------------------|
| Typed engine DTO in `@forge/contracts` (F0.1) | A **consumable, machine-readable Crucible wire DTO schema** to generate TS from (the `cdb-wire` `WireRequest`/`WireReply` + CrucibleQL row/DTO shapes). | **crdb -- DONE** (`IP-CONSOLE-READINESS` Part A, `INV-WIRE-DTO-SCHEMA-EXPORTED`, CR.A1/CR.A2, crdb `main`): the DTO contract is emitted as a drift-gated JSON Schema, committed at `crates/cdb-wire/schema/wire-dto.schema.json`. **F0.1 generates `@forge/contracts` directly from that artifact -- no hand-authoring needed.** |
...
| Live-feel surfaces (F0.6) | Fresh state on the Overview graph / Logs / AIOps feeds (`INV-CONSOLE-LIVE`, < 2 s). | **v1: CrucibleQL polling (needs no new engine work); push-stream DEFERRED.** A governed push-stream is an optimization, not a Phase-0 prerequisite. `IP-CONSOLE-READINESS` Part B landed the wire contract (CR.B1) + the bounded, credit-gated fan-out core (CR.B2a) and is **paused/banked** (product decision 2026-07-09); CR.B2b/B3/B4/B.N deferred. **F0.6 ships a short-interval CrucibleQL polling channel** (`AS OF`/incremental over `QuerySubmit`, which exists) behind the "Live" badge + staleness marker, hitting < 2 s freshness without the engine push-stream. The true push-stream is a later upgrade for the one surface (realistically the Overview connectivity graph at scale) that proves polling insufficient, at which point Part B resumes. Also serves `CONSOLE-09` / `CONSOLE-07`. |
...
**Foundation status against the engine (2026-07-09).** The one hard dependency -- a codegen-able DTO
schema (F0.1) -- is **DONE**: crdb `IP-CONSOLE-READINESS` Part A shipped the drift-gated JSON Schema, so
F0.1 generates `@forge/contracts` from a committed artifact with no hand-authoring. Everything else the
foundation needs **already exists**: CrucibleQL reads (`QuerySubmit`), the admin/command surface
(`cdb-actl`), the enrolled mTLS `:7878` seam. The one deliberately **deferred** item is the push-stream
(F0.6): a governed streaming subsystem is an `INV-CONSOLE-LIVE` optimization, not a prerequisite, so v1
serves live-feel by short-interval CrucibleQL polling and the push-stream (crdb Part B, contract + fan-out
core banked) resumes only when a concrete surface proves polling insufficient. So no foundation step is
blocked on unbuilt engine work, and no fabricated data is required anywhere.
