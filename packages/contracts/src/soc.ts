...
// ── The incident report (crdb IP-AISOC-STEP1 C.5, `SOC_INCIDENT_REPORT`; Console IP-CONSOLE-03 S3.16) ──

/** The six report sections, in the engine's order (crdb `ReportSectionName`). */
...
/** Who filled a section: the model's adjudicated prose, the engine's facts, or the declared template. */
export const REPORT_SECTION_SOURCES = ['model', 'engine', 'template'] as const;
export type ReportSectionSource = (typeof REPORT_SECTION_SOURCES)[number];

/** The narrative's state behind the report (crdb `NarrativeState`). */
export const REPORT_NARRATIVE_STATES = ['published', 'refused', 'absent'] as const;
export type ReportNarrativeState = (typeof REPORT_NARRATIVE_STATES)[number];

export interface ReportSection {
  readonly name: ReportSectionName;
  readonly source: ReportSectionSource;
  readonly lines: readonly string[];
}

/** One incident's shaped report: every section states its source; nothing here was generated by the read. */
export interface SocReport {
  readonly incidentId: string;
  readonly generatedAt: number;
  readonly narrativeState: ReportNarrativeState;
  /** The refusal reason or the absence reason (`unbound` / `not_run`); null when published. */
  readonly narrativeDetail: string | null;
...
function toReportSection(wire: WireReportSection): ReportSection | null {
  const name = narrowTag(REPORT_SECTION_NAMES, wire.name);
  const source = narrowTag(REPORT_SECTION_SOURCES, wire.source);
  if (name === null || source === null) {
    return null;
  }
  return { name, source, lines: wire.lines };
}

/**
 * Project the `SOC_INCIDENT_REPORT` reply. FAIL-CLOSED: `null` for a refusal, an unknown section
 * name or source, an unknown narrative state, or a report that carries a section twice -- a
 * template section rendered under a "model" label would claim an adjudication that never happened.
 */
export function toSocReport(wire: WireSocReport): SocReport | null {
  if (wire.refused) {
    return null;
  }
  const narrativeState = narrowTag(REPORT_NARRATIVE_STATES, wire.narrative_state);
  if (narrativeState === null) {
    return null;
  }
  const sections: ReportSection[] = [];
  const seen = new Set<string>();
  for (const wireSection of wire.sections) {
    const section = toReportSection(wireSection);
    if (section === null || seen.has(section.name)) {
      return null;
    }
  ...
  return {
    incidentId: wire.incident,
    generatedAt: wire.generated_at,
    narrativeState,
    narrativeDetail:
      wire.narrative_detail === undefined || wire.narrative_detail === ''
        ? null
        : wire.narrative_detail,
    modelRef: wire.model_ref === undefined || wire.model_ref === '' ? null : wire.model_ref,
    inputHash: wire.input_hash,
    sections,
    citedEvidence: wire.cited_evidence,
    needsHumanReview: wire.needs_human_review,
  };
}

/** The operator's words for a section heading. */
export function reportSectionLabel(name: ReportSectionName): string {
  switch (name) {
...
/** The operator's words for a section's source. */
export function reportSourceLabel(source: ReportSectionSource): string {
  switch (source) {
    case 'model':
    ...
    case 'template':
      return 'template (declared fallback)';
  }
}

/**
 * Render the report as plain text for export: the engine's sections verbatim, each under its heading
 * with its source declared. Nothing is added, reordered, or summarized -- the file IS the read.
 */
export function reportToText(report: SocReport): string {
  const head = [
    `Incident ${report.incidentId}`,
    `Generated at ${new Date(report.generatedAt * 1000).toISOString()}`,
    `Narrative: ${report.narrativeState}${report.narrativeDetail === null ? '' : ` (${report.narrativeDetail})`}`,
    `Model: ${report.modelRef ?? 'none bound'}`,
    `Input hash: ${report.inputHash}`,
    `Needs human review: ${report.needsHumanReview ? 'yes' : 'no'}`,
  ];
  const body = report.sections.map((section) =>
    [
      `## ${reportSectionLabel(section.name)} [${reportSourceLabel(section.source)}]`,
